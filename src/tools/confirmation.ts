...
export const requestConfirmationTool: Tool = {
  ...
  description:
    "Request safe word confirmation for a dangerous action. Creates a pending confirmation and DMs the user. Use this in GROUP chats when a dangerous action needs the safe word — it stores the action so the DM chat can verify and execute it.",
  parameters: {
    type: "object",
    properties: {
      dm_id: {
        type: "string",
        description: "The user's DM-able ID (phone number, username, etc.)",
      },
      action_description: {
        type: "string",
      ...
      tool_name: {
        type: "string",
        description: "The tool to execute once confirmed",
      },
      tool_input: {
        type: "string",
        description: "JSON string of the tool input to execute once confirmed",
      },
    },
  ...
  async execute(
    ...
    return {
      output: `Confirmation #${row.id} created. Now DM the user at ${input.dm_id} asking for the safe word and referencing confirmation #${row.id}.`,
    };
  },
};
...
export const checkPendingConfirmationsTool: Tool = {
  ...
  description:
    "Check pending confirmations waiting for this user's safe word. Use in DM chats when a user may be responding to a safe word request.",
  parameters: {
    type: "object",
    properties: {
...
export const approveConfirmationTool: Tool = {
  ...
  description:
    "Approve a pending confirmation by providing the safe word. The safe word is verified server-side before executing the action.",
  parameters: {
    type: "object",
    properties: {
      confirmation_id: {
        type: "number",
        description: "The confirmation ID to approve",
      },
      safe_word: {
        type: "string",
  ...
  async execute(
    input: { confirmation_id: number; safe_word: string },
    ctx: ToolContext,
  ): Promise<ToolResult> {
    ...
    if (!row)
      return {
        output: "Confirmation not found or already resolved.",
        isError: true,
      };
...
export const confirmationTools = [
  ...
  checkPendingConfirmationsTool,
  approveConfirmationTool,
  denyConfirmationTool,
];
