  ...
  parameters: {
    ...
    properties: {
      timezone: {
        type: "string",
        description:
          "IANA timezone (e.g., 'America/New_York'). Default: system timezone.",
      },
    },
  },
  risk: "low",
...
export const todoTool: Tool = {
  name: "todo",
  description:
    "Manage a simple todo list for the current chat. Actions: list, add, complete, remove.",
  parameters: {
    type: "object",
    properties: {
      action: {
        type: "string",
        enum: ["list", "add", "complete", "remove", "edit"],
        description: "Action to perform",
      },
      text: { type: "string", description: "Todo text (for add/edit)" },
      index: {
        type: "number",
        description: "Todo index (for complete/remove/edit, 1-based)",
      },
    },
    required: ["action"],
  },
  ...
  async execute(
    input: { action: string; text?: string; index?: number },
    ctx: ToolContext,
  ): Promise<ToolResult> {
    const key = `todos_${ctx.chatId}`;
    const raw = ctx.db
      .query("SELECT value FROM db_meta WHERE key = ?")
      .get(key) as { value: string } | null;
    const todos: Array<{ text: string; done: boolean }> = raw
    ...
    ctx.db.run("INSERT OR REPLACE INTO db_meta (key, value) VALUES (?, ?)", [
      key,
...
export const exportChatTool: Tool = {
  ...
  parameters: {
    ...
    properties: {
      filename: {
        type: "string",
        description: "Output filename (default: chat_export.md)",
      },
      limit: {
        type: "number",
  ...
  async execute(
    input: { filename?: string; limit?: number },
    ctx: ToolContext,
  ): Promise<ToolResult> {
...
export const calculateTool: Tool = {
  name: "calculate",
  description:
    "Evaluate a mathematical expression. Supports basic arithmetic, Math functions, etc.",
  parameters: {
    type: "object",
    properties: {
      expression: {
        type: "string",
        description: "Math expression (e.g., '2 + 3 * 4', 'Math.sqrt(144)')",
      },
    },
    required: ["expression"],
  },
  ...
  async execute(input: { expression: string }): Promise<ToolResult> {
    try {
      const sanitized = input.expression.replace(
        /[^0-9+\-*/().%^eE ,Math.sqrtpowabsceilfloorlogroundminmaxPIsincostan]/g,
        "",
      );
...
export const listChatsTool: Tool = {
  name: "list_chats",
  description:
    "List known chats across all channels. Useful for finding chat IDs for cross-channel messaging.",
  parameters: {
    type: "object",
    properties: {
      channel: {
        type: "string",
        description: "Filter by channel (e.g., 'signal', 'discord')",
      },
  ...
  async execute(
    ...
    let query =
      "SELECT id, channel, external_chat_id, chat_type, title, created_at FROM chats";
    const params: any[] = [];
    if (input.channel) {
      query += " WHERE channel = ?";
...
export const verifySafeWordTool: Tool = {
  ...
  description:
    "Verify the user's safe word before performing a dangerous action. Returns whether the provided word matches.",
  parameters: {
    type: "object",
    properties: {
      word: {
        type: "string",
        description: "The safe word provided by the user",
      },
    },
    required: ["word"],
  },
...
export const manageAllowedUsersTool: Tool = {
  ...
  parameters: {
    ...
    properties: {
      ...
      channel: {
        type: "string",
        description: "Channel (e.g., 'signal', 'discord', 'slack', 'imessage')",
      },
      user_id: {
        type: "string",
        description:
  ...
  async execute(
    ...
    if (input.action === "list") {
      const rows = ctx.db
        .query(
          "SELECT user_id, added_by, created_at FROM allowed_users WHERE channel = ?",
        )
        .all(input.channel) as any[];
      if (rows.length === 0)
        return {
          output: `No allowed users for ${input.channel} (all users permitted).`,
        };
      return {
        output: rows
          .map(
            (r: any) =>
              `${r.user_id} (added by: ${r.added_by}, ${r.created_at})`,
          )
          .join("\n"),
      };
    }
    ...
    if (!input.user_id)
      return { output: "user_id is required for add/remove", isError: true };
    ...
    if (input.action === "add") {
      ...
      return {
        output: `Added ${input.user_id} to ${input.channel} allowlist.`,
      };
    }
    ...
    if (input.action === "remove") {
      ...
      return {
        output: `Removed ${input.user_id} from ${input.channel} allowlist.`,
      };
    }
...
export const readChatHistoryTool: Tool = {
  ...
  description:
    "Read recent messages from any chat by its ID. Use list_chats to find chat IDs. Useful for cross-referencing conversations or checking what was said in another chat.",
  parameters: {
    type: "object",
    properties: {
  ...
  async execute(
    input: { chat_id: number; limit?: number },
    ctx: ToolContext,
  ): Promise<ToolResult> {
    const limit = input.limit || 20;
    const messages = getRecentMessages(ctx.db, input.chat_id, limit);
    if (messages.length === 0)
      return { output: "No messages found in that chat." };
